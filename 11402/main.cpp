...
void push(int x){
    ...
    if(node[x].z2 != 0 ){
        ...
        node[lson(x)].v = (node[lson(x)].r-node[lson(x)].l) - node[lson(x)].v;
        node[rson(x)].v = (node[rson(x)].r-node[rson(x)].l) - node[rson(x)].v;
    }
    if(node[x].z1 != 0 ){
        ...
        node[lson(x)].v = (node[lson(x)].r-node[lson(x)].l) * node[lson(x)].z1;
        node[rson(x)].v = (node[rson(x)].r-node[rson(x)].l) * node[rson(x)].z1;
    }
}
...
void interMod1(int a, int b, int m, int x = 1){
    ...
    */
    ...
    if(a <= node[x].l && b >= node[x].r){ //this node interval will be recover
        node[x].v = (node[x].r-node[x].l) * m; //m is 1 or 0
        node[x].z1 += m;
        return;
    }
    push(x);
    ...
    if(b <= mid) interMod1(a, b, m, lson(x));
    if(a > mid) interMod1(a, b, m, rson(x));
...
void interMod2(int a, int b, int m, int x = 1){
    ...
    */
    ...
    if(a <= node[x].l && b >= node[x].r){ //this node interval will be recover
        node[x].v = (node[x].r-node[x].l) - node[x].v; //m is 1 or 0
        node[x].z2 += m;
        return;
    }
    push(x);
